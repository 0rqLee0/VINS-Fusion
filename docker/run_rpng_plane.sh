#!/bin/bash
# 一键运行 VINS-Fusion rpng_plane (table) 数据集 (单目+IMU) 并保存 TUM 格式轨迹
#
# 用法: ./run_rpng_plane.sh <bag文件路径> [选项]
# 示例:
#   ./run_rpng_plane.sh /root/datasets/rpng_plane/table.bag
#   ./run_rpng_plane.sh /root/datasets/rpng_plane/table.bag --loop --rviz
#
...
# 默认参数
CONFIG="rpng_plane_mono_imu_config"
...
if [[ -z "$BAG_FILE" ]]; then
    echo "错误: 请指定bag文件路径"
    echo "用法: ./run_rpng_plane.sh <bag文件路径> [--config <配置名>] [--loop] [--rviz]"
    exit 1
fi
...
echo "========================================"
echo " VINS-Fusion rpng_plane 自动运行"
echo "========================================"
echo " 配置:     $CONFIG"
echo " Bag:      $BAG_FILE"
...
# 配置文件路径
CONFIG_DIR="/root/catkin_ws/src/VINS-Fusion/config/rpng_plane"
...
# 运行 launch
roslaunch vins rpng_plane_run.launch \
